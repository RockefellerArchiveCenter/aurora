...
class OrganizationDetailView(RACUserMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['uploads'] = Archives.objects.filter(process_status>=20, organization = context['object']).order_by('-created_time')[:15]
        context['uploads_count'] = Archives.objects.filter(process_status>=20, organization = context['object']).count()
        return context
...
class OrganizationTransfersView(RACUserMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return Archives.objects.filter(process_status>=20, organization=self.organization).order_by('-created_time')
...
class UsersDetailView(SelfOrSuperUserMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['uploads'] = Archives.objects.filter(process_status>=20, organization = context['object'].organization).order_by('-created_time')[:5]
        context['uploads_count'] = Archives.objects.filter(process_status>=20, organization = context['object'].organization).count()
        return context
