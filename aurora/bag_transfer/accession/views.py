...
class AccessionRecordView(AccessioningArchivistMixin, View):
    ...
    def package_transfer(self, transfer, request):
        tar_dir = join(settings.DELIVERY_QUEUE_DIR, transfer.machine_file_identifier)
        tarfilepath = "{}package_{}.tar.gz".format(settings.DELIVERY_QUEUE_DIR, transfer.machine_file_identifier)
        ...
        with tarfile.open(join(tar_dir, '{}.tar.gz'.format(transfer.machine_file_identifier)), "w:gz") as bagtar:
            ...
            bagtar.close()
        # zip up the package
        with tarfile.open(join(settings.DELIVERY_QUEUE_DIR, '{}.tar.gz'.format(transfer.machine_file_identifier)), "w:gz") as tar:
